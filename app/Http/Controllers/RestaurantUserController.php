...
class RestaurantUserController extends Controller
{
    ...
    {
        return Restaurant::withoutGlobalScope(ActiveScope::class)->with(["user.userDetails"=>function($r){
            $r->where("user_type","restaurant");
        }])->get();
    }
    ...
    public function filterAndPaginate(Request $request)
    {
        ...
        $validKeyFilterRest=['title_en'];
        $validKeyFilterUserDetails=['last_login'];
        try {
            $data =Restaurant::withoutGlobalScope(ActiveScope::class)->with(["user.userDetails"=>function($r){
                $r->whereIn("user_type",["restaurant","branch"]);
            }]);
            if(isset($filterKeyVal["key"])){

                if(in_array($filterKeyVal["key"],$validKeyFilterRest)){
                ...
                if(in_array($filterKeyVal["key"],$validKeyFilterUserDetails)){
                    $data=$data->whereHas("user.userDetails",function ($r)use($filterKeyVal){
                        $r->where($filterKeyVal["key"],'like',"%".$filterKeyVal['value']."%");
                    });
                }
            }
